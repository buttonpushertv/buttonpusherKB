...
halfTheKeys := round(section3_keys / 3)
section3GroupH := (halfTheKeys)
currentAltCounter := 1
Gui, Add, GroupBox, R%section3GroupH% x10 yp+20 w430 , Apps to Load
...
loop, %section3_keys%
  {
    ...
    ;MsgBox %currentKeyLeft7%
    If (currentKeyLeft7 = "loadApp") {
      ;MsgBox pathKey: %pathKey%`npathLookAhead:%pathLookAhead%`nnameKey: %nameKey%`nnameLookAhead:%nameLookAhead%`ncurrentKeyValue: %currentKeyValue%`ncurrentPathValue: %currentPathValue%`ncurrentNameValue: %currentNameValue%
...
Gui, Font, S8 CDefault, Franklin Gothic Medium
;Gui, Add, Text, xp yp+25, GUIHeight:%guiHeight% | KeyRows:%keyRows% | buttonStartingY:%buttonStartingY%
Gui, Font, S12 CDefault, Franklin Gothic Medium
...
Gui, Font, S8 CDefault, Franklin Gothic Medium
Gui, Add, Text, x35 yp+55 vtimeoutText, %timeoutText%
Gui, Add, Text, x275 yp w25 vtimeoutTextProgress
Gui, Font, S12 CDefault, Franklin Gothic Medium
Gui, Add, Progress, x10 yp+20 w430 h20 cGreen BackgroundNavy Range0-%timeoutSegments% vtimeoutProgress
Gui, Add, Button, x10 yp+40 w430 h30, Launch Apps &Now
;guiHeight += 75
;Gui, Add, Button, x10 yp+40 w430 h30, Variables
Gui, Show, w450 h%guiHeight%
...
loop, %section3_keys%
{
    ...
    currentPathValue := %section3%_%pathKey%
    currentKeyLeft7 := SubStr(currentKey, 1, 7)
    ;MsgBox %currentKeyLeft7%`n%currentKeyValue%
    If (currentKeyLeft7 = "loadApp") {
        If (currentKeyValue) {
...
ButtonAddApp:
InputBox, OutputVar, File Name, App to Add`n(include path relative to LAUNCHER.AHK)`n%A_ScriptDir%
if OutputVar=
  return
numberOfKeySets := round(section3_keys / 2)
...
Apps_pathApp%newNumberOfKeySets% := OutputVar
section3_keys := (newNumberOfKeySets * 2)
INI_Save(inifile)
reload
return
