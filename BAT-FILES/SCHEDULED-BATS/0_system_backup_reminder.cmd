...
IF NOT EXIST %rootFolder%\PRIVATE\%computername%\days_since_system_backup.txt echo 15>%rootFolder%\PRIVATE\%computername%\days_since_system_backup.txt
...
for /f "delims=" %%a in (%rootFolder%\PRIVATE\%computername%\days_since_system_backup.txt) do set /a daysSince=%%a

@REM The line below gets the date the days_since_system_backup.txt file was last modified.
for %%a in (%rootFolder%\PRIVATE\%computername%\days_since_system_backup.txt) do set fileDate=%%~ta
...
if /I %daysSince% GEQ %checkWindow% (call "%rootFolder%\BAT-FILES\launch System-Backup.cmd" & EXIT /b)
...
@echo %backupDays% > %rootFolder%\PRIVATE\%computername%\days_since_system_backup.txt
