...
IF NOT EXIST %BKB_ROOT%\PRIVATE\%computername%\days_since_system_backup.txt echo 15>%BKB_ROOT%\PRIVATE\%computername%\days_since_system_backup.txt
...
for /f "delims=" %%a in (%BKB_ROOT%\PRIVATE\%computername%\days_since_system_backup.txt) do set /a daysSince=%%a

@REM The line below gets the date the days_since_system_backup.txt file was last modified.
for %%a in (%BKB_ROOT%\PRIVATE\%computername%\days_since_system_backup.txt) do set fileDate=%%~ta
...
if /I %daysSince% GEQ %checkWindow% (call "%BKB_ROOT%\BAT-FILES\launch System-Backup.cmd" & EXIT /b)
...
@echo %backupDays% > %BKB_ROOT%\PRIVATE\%computername%\days_since_system_backup.txt
